...
function validateDetectConfig(tech) {
  ...
  if (detect.packages !== undefined) {
    ...
    for (const pkg of detect.packages) {
      ok(isNonEmptyString(pkg), `[${tech.id}] detect.packages contains an empty or non-string value`)
    }
  }

  if (detect.packagePatterns !== undefined) {
    ok(Array.isArray(detect.packagePatterns), `[${tech.id}] detect.packagePatterns must be an array`)
    ok(
  ...
  if (detect.configFiles !== undefined) {
    ...
    for (const file of detect.configFiles) {
      ok(isNonEmptyString(file), `[${tech.id}] detect.configFiles contains an empty or non-string value`)
    }
  }
...
describe("validation gives actionable feedback on bad entries", () => {
  ...
  it("reports empty detect by listing all available signal types", () => {
    expectsError(
      () =>
        validateTechnologyEntry({ id: "my-tech", name: "My Tech", detect: {}, skills: [VALID_SKILL] }),
